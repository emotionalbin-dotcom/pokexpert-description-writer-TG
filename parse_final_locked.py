...
if os.path.exists('research_data.json'):
    try:
        with open('research_data.json', 'r', encoding='utf-8') as _rf:
            _rd = json.load(_rf)
        _special = _rd.get('special_research', [])
        if _special:
            lines.append("🔬 𝗦𝗳𝗲𝗰𝗶𝗮𝗹 𝗥𝗲𝘀𝗲𝗮𝗿𝗰𝗵 𝗤𝗹𝗲𝘀𝘁𝘀")
            lines.append("")
            for _q in _special:
                _title = _q.get('title', 'Unknown')
                _status = _q.get('status', '')
                _icon = "✅" if 'COMPLETE' in str(_status).upper() else "🟡" if _q.get('completed_steps', 0) > 0 else "🔴"
                lines.append(f"{_icon} {_title}")
            lines.append("")
